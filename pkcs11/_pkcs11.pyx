...
cdef class MechanismWithParam:
    ...
    def __init__(self, key_type, mapping, mechanism=None, param=None):
        ...
        if mechanism in (Mechanism.AES_ECB_ENCRYPT_DATA,
            ...
            # CKM_HKDF_KEY_GEN with no params = simple key generation
            if param is None and mechanism == Mechanism.HKDF_KEY_GEN:
                pass  # No CK_HKDF_PARAMS needed for key generation
            else:
...
cdef class Session(HasFuncList, types.Session):
    ...
    def generate_key(self, key_type, key_length=None,
        ...
        # Build attributes
        if key_type not in (KeyType._DES, KeyType.DES2, KeyType.DES3, KeyType.GOST28147, KeyType.SEED):
